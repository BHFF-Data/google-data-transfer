...
from google_data_transfer.google_api.auth import authenticate_google_api_st
from google_data_transfer.google_api.google_client import GoogleFormsClient


@dataclass
class RawFormData:
    contents_and_metadata: dict
    responses: list
...
class GoogleAPIForm(Form):
    ...
    def __init__(self, form_url: str, google_form_client: GoogleFormsClient):
        ...
        Args:
            ...
            google_form_client: Connection to Google Forms
        ...
        self._google_conn = google_form_client
    ...
    def from_creds_file(
        ...
        forms_conn = GoogleFormsClient(creds)
        return cls(form_url, forms_conn)

    def _read_form_raw_data(self) -> RawFormData:
        """!!! note
        ...
        raw_data = RawFormData(
            contents_and_metadata=self._google_conn.fetch_form_contents_metadata(
                self.id
            ),
            responses=self._google_conn.fetch_form_responses(self.id)["responses"],
        )
        return raw_data
    ...
    def _join_questions_and_responses_to_df(
        self, contents: dict, responses: list
    ) -> pd.DataFrame:
        """
        Args:
            raw_form_data:
    ...
    def to_df(self) -> pd.DataFrame:
        ...
        df = self._join_questions_and_responses_to_df(
            raw_data.contents_and_metadata, raw_data.responses
        )
        return df
    ...
    def from_creds(
        ...
        !!! note
            ...
            The above docstring is autogenerated by docstring-gen library (https://docstring-gen.airt.ai)
        """
        creds = authenticate_google_api_st(json_token, scopes)
        forms_conn = GoogleFormsClient(creds)
        return cls(form_url, forms_conn)
