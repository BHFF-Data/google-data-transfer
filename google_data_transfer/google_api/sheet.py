...
class GoogleSheet:
    ...
    def __init__(self, gspread_client: gspread.client.Client, spreadsheet_url: str):
        ...
        Raises:
            ValueError: If gspread_client or spreadsheet_url is None
        """
        if gspread_client is None or spreadsheet_url is None:
            raise ValueError(
                "Both gspread_client and spreadsheet_url must be provided."
            )
    ...
    def _get_subsheets(self) -> list[gspread.Worksheet]:
        """Get a dictionary of subsheets in the spreadsheet."""
        return self._sheet.worksheets()

    def get_subsheet_names(self) -> list[str]:
        """Get names of all subsheets."""
        subsheets = self._get_subsheets()
        return [subsheet.title for subsheet in subsheets]

    def get_subsheet(self, subsheet_name: str):
        """Get a specific subsheet by name."""
        gspread_subsheet = self._sheet.worksheet(subsheet_name)
        subsheet = GoogleSubSheet(gspread_subsheet)
        return subsheet
...
class GoogleSubSheet(SubSheet):
    ...
    def __init__(self, gspread_worksheet: gspread.Worksheet):
